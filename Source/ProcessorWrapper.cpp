...
void ProcessWrapper<SampleType>::createParameterLayout(std::vector<std::unique_ptr<RangedAudioParameter>>& params)
{
    ...
    auto fString = juce::StringArray({ "Low Pass", "High Pass", "Band Pass", "Peak", "Notch", "All Pass", "Low Shelf", "High Shelf", "Band Pass (g)", "Low Pass (1)", "High Pass (1)", "Low Shelf (1)", "Low Shelf (1c)", "High Shelf (1)", "High Shelf (1c)" });
